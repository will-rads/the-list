...
- `Package.swift` — `TheListCore`: Models + Services + tests as a PORTABLE SPM package.
  ...
  including Windows and Linux. GitHub Actions remains the canonical verification gate.
...
GitHub Actions is the compile and test gate. The iOS workflow runs `xcodegen`, `swift test`, and a full `xcodebuild build` on macOS. The local Windows Swift toolchain is no longer required.
...
CI compiles the app, but the simulator and device experience still need a Mac-day pass. Known
...
393×852 viewport (member = local `web/v3/index.html`, venue = local `web/v3/venue.html`).
